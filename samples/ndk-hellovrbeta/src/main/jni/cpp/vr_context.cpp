...
//    packet->controller_Pose_Orientation.x = 0;
//    packet->controller_Pose_Orientation.y = 0;
//    packet->controller_Pose_Orientation.z = 0;
//    packet->controller_Pose_Orientation.w = 0;

    LOG("Q %f %f %f %f", packet->controller_Pose_Orientation.x, packet->controller_Pose_Orientation.y, packet->controller_Pose_Orientation.z , packet->controller_Pose_Orientation.w);
...
//    packet->controller_Pose_Position.x = 0;
//    packet->controller_Pose_Position.y = 0;
//    packet->controller_Pose_Position.z = -2;

    LOG("P %f %f %f", p.x, packet->controller_Pose_Position.y, p.z);
//    LOG("P %f %f %f", p.x, packet->controller_Pose_Position.y, packet->controller_Pose_Position.z);
...
Java_com_polygraphene_alvr_VrContext_initializeNative(JNIEnv *env, jobject instance, jlong nativeGvrContext) {
    ...
    controllerApi->Init(gvr::ControllerApi::DefaultOptions() | GVR_CONTROLLER_ENABLE_ARM_MODEL | GVR_CONTROLLER_ENABLE_POSITION | GVR_TRACKING_STATUS_FLAG_HAS_6DOF, (gvr_context*)nativeGvrContext);
